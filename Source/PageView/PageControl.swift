...
public class PageControl: UIControl {
    ...
    public func setFillColor(_ fillColor: UIColor? = UIColor.white, for state: UIControl.State = .selected) {
...
extension PageControl {
   ...
   public func setupPageControlStyle(style: PageControlStyle, for state: UIControl.State) {
        let width = self.itemSpacing
        let height = self.itemSpacing
        var path = UIBezierPath()
        switch style {
        case .round:
            path = UIBezierPath(ovalIn: CGRect(x: 0, y: 0, width: self.itemSpacing, height: self.itemSpacing))
            break
        case .ellipse:
            path = UIBezierPath(roundedRect: CGRect(x: -self.itemSpacing/2, y: 0, width: width * 2, height: height), cornerRadius: 5.0)
            
            break
        }
        self.setPath(path, for: .selected)
    }
}
