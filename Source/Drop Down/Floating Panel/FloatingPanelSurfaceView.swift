...
public class FloatingPanelSurfaceView: UIView {
    ...
    public var cornerRadius_: CGFloat = 0.0 { didSet { setNeedsLayout() } }
    ...
    public override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        let translatedPoint = grabberHandle.convert(point, from: self)
        
        if (grabberHandle.bounds.contains(translatedPoint)) {
            isGrabberHit = true
        }
    ...
    private func updateLayers() {
        ...
        backgroundView.layer.cornerRadius = cornerRadius_
    ...
    private func updateContentViewMask() {
        if #available(iOS 11, *) {
            ...
            contentView?.layer.cornerRadius = cornerRadius_
